...
export default class MyCurrentApply extends React.Component {
  ...
  callArticles1 = () => {
    AsyncStorage.getItem("pubKey").then(async (pubKey) => {
      const {
        data: { result }
      } = await axios.post("http://70.12.225.80:4000/myapplylist", {
        params: {
          db_pubkey: pubKey
        }
      });
  ...
  callArticles2 = () => {
    AsyncStorage.getItem("pubKey").then(async (pubKey) => {
      const {
        data: { result }
      } = await axios.post("http://70.12.225.80:4000/jobSeekerList", {
        params: {
          db_pubkey: pubKey
        }
      });
  ...
  approve1 = async (db_articleId) => {
    alert("계약이 체결되었습니다.");
    await axios.post("http://70.12.225.80:4000/myapplylist/accept", {
      params: {
        db_accept: 1,
  ...
  approve2 = async (id) => {
    ...
    await axios.post("http://70.12.225.80:4000/jobSeekerList/accept", {
      params: {
        accept: 0,
        articleId: id
      }
